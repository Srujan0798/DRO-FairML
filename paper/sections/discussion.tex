...
separately testing an additive (Laplace-style) shrinkage of
...
and $\alpha$-dependently instead of imposing one fixed ceiling. We note
honestly that LSAC's validation split is large ($n{=}2804$) and its
minority proportion ($\approx6.4\%$) is measured, not noisily estimated,
so shrinkage's usual statistical justification (denoising a small
sample) does not straightforwardly apply here; we test it as a
regularizer in its own right, and report the result --- fixed or not
--- rather than tuning until it works.
...
temperature. A softer alternative --- a quadratic penalty on $\lambda$
in the Lagrangian, i.e.\ an augmented-Lagrangian formulation --- would
replace this tuned constant with a standard, well-understood
stabilization mechanism and is a natural target for follow-up work.
