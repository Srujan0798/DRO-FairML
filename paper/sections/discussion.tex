...
made at $\alpha\ge0.3$, where both methods fall below the
constant predictor).  At $\alpha\le0.2$ the gap widens monotonically.
...
The empirical comparison (adversarial-vs-random runs)
...
\subsection{Image data (UTKFace) --- real features; claim deferred}
The earlier ``DRO inverts on ResNet18 features'' claim from prior
versions of this paper is withdrawn (synthetic smoke only).  Real
features and a partial REAL-tagged grid exist locally
(\texttt{results/utkface\_canonical.json}); we defer any image-modality
result until the full protocol grid is complete and reviewed.
See \S\ref{sec:results-utkface}.
