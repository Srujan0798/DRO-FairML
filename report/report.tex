...
beating Naive-FAIR by up to $-57\%$.  The previous \"DRO fragile\" failure
...
(DP attack, mean of 3 seeds):
\begin{center}
\begin{tabular}{lccc}
\toprule
$\tau$ & Naive DP & DRO DP & Verdict \\
\midrule
1   & 0.248 & \cellcolor{wingreen}\textbf{0.237} & DRO wins \\
10  & 0.338 & \cellcolor{lossred}0.463            & Naive wins \\
100 & 0.327 & \cellcolor{lossred}0.503            & Naive wins \\
\bottomrule
\end{tabular}
\end{center}
...
attack is weak in absolute terms because LSAC's clean DP is small.

\subsection{IF k-NN Ablation}

Re-running the IF attack with $k{\in}\{5,10,15\}$ neighbors gives
near-identical attack strength and DRO/Naive gap.  IF and DP values
across $k$ are within ${\pm}0.003$ of each other at every $\alpha$, so
$k{=}5$ is a safe default and the IF attack is robust to graph choice.
...
\subsection{LSAC Framing (Q3)}

LSAC has inherently small clean DP ($\approx 0.02$ on Sex) so the DP
attack cannot raise it much --- the IF attack is the more sensitive
metric.  This is not a bug; it is a consequence of LSAC's near-zero
baseline DP.  The LSAC tau=1 numbers are in progress (270-row re-run);
we will frame LSAC around the IF attack per Kuldeep's~Q3.
...
attack structure is known.  We retain the closed-form as the default and
