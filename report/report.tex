...
\item \textbf{UTKFace partial (no paper claim yet).}
  Real features exist (\texttt{data/raw/utkface\_features.npz}, $N{=}23{,}705$);
  a REAL-tagged grid is accumulating in \texttt{results/utkface\_canonical.json}
  (DP complete; IF then Combined; target 90 configs).  Prior synthetic smoke
  numbers and the ``DRO inverts on image features'' claim remain withdrawn
  until the full multi-seed multi-attack grid is complete and reviewed.
  flair2 is staged infrastructure only---not a dependency.
\end{enumerate}
...
\item \textbf{UTKFace (partial REAL grid only):} Real ResNet features are
  on disk; \texttt{results/utkface\_canonical.json} has a partial REAL-only
  run (attack=\texttt{dp} so far).  No multi-seed full image claim yet.
  Prior synthetic smoke ``DRO inverts on image features'' numbers remain
  withdrawn.
\end{itemize}
...
synthetic smoke tests withdrawn). Q5 empirical radii math in appendix (see also \texttt{docs/KULDEEP\_CORRECTION.md}).
