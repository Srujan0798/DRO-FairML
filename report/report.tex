...
Seeds: 0--2 (3 per configuration for tau=1 runs; 3 additional seeds
in progress for $n{=}6$ significance).  Adult tau=1 complete (90
configurations); Credit/LSAC tau=1 in progress (270-row target).
...
\paragraph{Key highlights (fixed $\tau{=}1$, Adult, 3 seeds).}
\begin{itemize}
\item \textbf{DP attack:} DRO beats Naive at every $\alpha$:
  $\alpha{=}0.2$ Naive $0.248$ vs DRO $0.237$;
  $\alpha{=}0.3$ Naive $0.286$ vs DRO $0.264$;
  $\alpha{=}0.4$ Naive $0.310$ vs DRO $0.283$.
\item \textbf{Combined attack:} DRO beats Naive at every $\alpha$:
  $\alpha{=}0.2$ Naive $0.199$ vs DRO $0.183$;
  $\alpha{=}0.3$ Naive $0.219$ vs DRO $0.195$;
  $\alpha{=}0.4$ Naive $0.213$ vs DRO $0.185$.
\item \textbf{DRO advantage grows with $\alpha$:} exactly as the theory
  predicts ($\alpha{=}0.1$: $-0.002$, $\alpha{=}0.4$: $-0.027$ DP gap).
...
\begin{itemize}
\item \textbf{At fixed $\tau{=}1$: DRO wins on Adult DP under all three attacks.}
  Under the DP-targeted attack DRO beats Naive at every $\alpha$ and the
  gap widens with $\alpha$ (see Table~\ref{tab:main_results}).
\item \textbf{IF-attack is the strongest attack on Credit/LSAC:}
  When the attacker targets Individual Fairness via $k$-NN gradients,
  DRO-FAIR's corruption-calibrated weights provide the most robust defense
  (64--97\% DP reduction on Credit and LSAC in the tau=100 runs, $p<0.05$).
...
\item \textbf{IF attack is $k$-insensitive:} Re-running with $k{\in}\{5,10,15\}$
  gives near-identical IF and DP values (${\pm}0.003$).
\item \textbf{GPU server issue:} Full UTKFace (200K image) experiments are
  blocked pending GPU server access.  The earlier ``DRO inverts on image
  features'' claim is withdrawn.
\end{itemize}
...
$\tau{=}1$ across all $\alpha$ makes DRO-FAIR beat Naive-FAIR on DP at
every corruption level on Adult}, with the advantage growing in $\alpha$
and accuracy equal-or-slightly-better.  The earlier ``DRO is fragile /
...
inverts that conclusion.  The IF attack is insensitive to the $k$ of the
$k$-NN graph ($k{\in}\{5,10,15\}$).
...
Credit and LSAC tau=1 numbers are in progress (270-row re-run running);
the Adult result is the mature finding in this report.  The UTKFace
experiment is blocked on GPU access and the earlier ``DRO inverts on
image features'' claim is withdrawn.
...
\begin{itemize}
\item Credit/LSAC tau=1 canonical numbers (270-row re-run completing now).
\item $n{=}6$ seeds for Wilcoxon $p{<}0.05$ (in progress).
\item Empirical radii calibration under known attack structure (Q5).
\item UTKFace on GPU (blocked on flair2 access).
...
109 tau=1 runs (Adult complete), 32 unit tests, 8 theory verifications.
}
