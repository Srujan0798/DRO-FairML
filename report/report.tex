...
On the canonical grid (fixed $\tau{=}1$, $k_{\mathrm{inner}}{=}10$,
$n{=}6$ seeds), DRO-FAIR achieves significantly lower DP than Naive-FAIR
on \textbf{Adult} and \textbf{Credit} at $\alpha\le0.2$ under the
DP-targeted and Combined attacks (Wilcoxon $p{<}0.05$), reproducing the
original Jun~30 report for Adult DP, with accuracy equal-or-better.
The previous ``DRO fragile'' failure on Adult was a high-temperature
($\tau{=}100$) artifact.  LSAC/DP is a degenerate negative result
(constant-predictor collapse, DRO loses at $0/6$ seeds); LSAC/Combined
is a genuine win ($p{=}0.0156$).  IF-attack results are pending a cluster
re-run (earlier IF metrics were degenerate, $\approx10^{-10}$).
...
$k_{\mathrm{inner}}{=}10$ grid.  The DP+Combined attack grid is complete
(360 rows: 3 datasets $\times$ 5 $\alpha$ $\times$ 6 seeds $\times$ 2
methods); the IF-attack third is pending a cluster re-run.
...
\begin{table}[H]
\centering\small
\caption{Main Results: DRO-FAIR vs Naive-FAIR under Adversarial Corruption (clean test, mean $\pm$ SE over 6 seeds, $\tau{=}1$ canonical).}
...
IF-attack third of the grid is pending a cluster re-run; earlier IF
...
\begin{figure}[H]
  ...
  \caption{\textbf{Win-Rate Summary.}
    ...
    at $\alpha\in\{0.1,0.2,0.3\}$, Wilcoxon $p<0.05$ (IF comparisons pending re-run).}
  \label{fig:winrates}
\end{figure}
...
IF-attack evaluation rows are pending a cluster re-run (the IF metric was
degenerate, $\approx10^{-10}$, in earlier runs); the pending re-run will
use this $k{=}5$ default.
...
\subsection{LSAC Framing (Q3) + Q7 (DP/IF coupling, pending)}
...
Q7: the IF-attack rows of the canonical grid are not yet generated (cluster
re-run pending after the IF-metric fix), so the earlier numeric IF claims
are withdrawn.  Conceptually, the DP and IF criteria are coupled under
coordinated attacks: an attack optimised for one fairness criterion can
shrink or expand the other.  This will be characterised once the IF-attack
third of the grid is regenerated. (See also paper/sections/results.tex.)
...
\item \textbf{Statistical significance.}
  ...
  grid is pending a cluster re-run (earlier IF metrics were degenerate).
\item \textbf{UTKFace blocked on GPU.}
  No real UTKFace image experiment has been run; the prior reported
  numbers were synthetic smoke tests (random Gaussian features).  The
  ``DRO inverts on image features'' claim from prior versions is
  withdrawn until real tau=1 UTKFace data exists.
\end{enumerate}
...
\item \textbf{IF attack is pending:} The IF-attack rows of the canonical grid
  are not yet generated (cluster re-run pending after the IF-metric fix);
  the earlier IF metrics were degenerate ($\approx10^{-10}$).  The $k$-NN
  graph is insensitive to $k{\in}\{5,10,15\}$ (DP within ${\pm}0.003$).
\item \textbf{GPU server issue:} UTKFace experiments are blocked pending
  GPU server access; the only UTKFace outputs to date are synthetic smoke
  tests (random Gaussian features), so the earlier ``DRO inverts on image
...
a genuine win ($p{=}0.0156$).  The IF-attack rows are pending a cluster
re-run (earlier IF metrics were degenerate, $\approx10^{-10}$).
...
The canonical DP+Combined grid is complete (360 rows); Adult and Credit
...
rows are pending a cluster re-run.  The UTKFace
experiment is blocked on GPU access (no real run to date; prior numbers
were synthetic smoke tests) and the earlier ``DRO inverts on image
features'' claim is withdrawn. Q5 empirical radii math in appendix (see also KULDEEP\_DISCUSSION.md).
...
\begin{itemize}
\item IF-attack third of the canonical grid (cluster re-run after the IF-metric fix).
...
Canonical $\tau{=}1$ grid complete (360 rows; \texttt{results/canonical\_tau1.json}), 32 unit tests, 8 theory verifications.
}
