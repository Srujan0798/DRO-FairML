...
A historical pilot (pre-canonical protocol) found fairness-targeted PGD
raises Adult/Credit DP far more than matched-$\alpha$ random label noise
(Kuldeep Q1 sanity: adversarial $\gg$ random).  We keep that qualitative
check but do \emph{not} treat older multiplicative ``$12$--$40\times$''
factors as locked under the 540-row $\tau{=}1$ protocol until a seed-paired
re-run is committed.  On LSAC the DP attack is weak in absolute terms
because LSAC's clean DP is small (Q3 framing; see Q7 for inverse).
