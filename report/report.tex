...
is a genuine win ($p{=}0.0156$).  IF-attack results are complete (180 rows;
see \texttt{results/if\_wilcoxon\_summary.txt}); earlier incomplete wording is
obsolete.  Pre-fix IF metrics were degenerate ($\approx10^{-10}$).
...
the IF-metric fix; see \texttt{results/if\_wilcoxon\_summary.txt}). Earlier
pre-fix numeric IF claims remain withdrawn.  Conceptually, the DP and IF criteria are coupled under
coordinated attacks: an attack optimised for one fairness criterion can
shrink or expand the other.  Coupling is characterised on the full IF-attack
...
a genuine win ($p{=}0.0156$).  The IF-attack rows are complete; under IF attack Adult/Credit $\alpha\le0.2$ still support DRO on DP while LSAC/IF is a DP loss (cluster
...
LSAC/IF is a DP loss --- not a three-attack sweep).  UTKFace has real
local features; a multi-seed grid is running and not claimed here (prior
synthetic smoke tests withdrawn). Q5 empirical radii math in appendix (see also KULDEEP\_DISCUSSION.md).
...
Canonical $\tau{=}1$ grid complete (540 rows; \texttt{results/canonical\_tau1.json}), 32 unit tests, 8 theory verifications.
}
