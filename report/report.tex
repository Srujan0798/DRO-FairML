...
\subsection{IF k-NN Ablation (scope)}

Canonical IF-attack rows use the default $k{=}5$ neighbourhood in the
attack construction.  A committed $k\in\{5,10,15\}$ ablation JSON is
\emph{not} currently in the tree, so we do not claim $k$-insensitivity as
a verified result.  The IF-attack evaluation rows themselves are complete
(180/180; the IF metric was degenerate, $\approx10^{-10}$, in earlier runs)
under this fixed $k{=}5$ default.
