...
toward balance (\texttt{pi\_shrinkage\_k} in \texttt{DroFairTrainer}) is
under test as an alternative to the per-group clamp that did not fix
LSAC --- reported honestly whichever way it lands. \textbf{(iii)
Augmented-Lagrangian dual stabilization}: replacing the tuned
$\lambda_{\max}$ hard clamp with a quadratic penalty on $\lambda$ removes
an empirically-chosen constant in favor of a standard mechanism. Full
discussion: \texttt{paper/sections/discussion.tex} (Future Work
subsection).
