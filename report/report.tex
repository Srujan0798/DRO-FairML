...
% ─────────────────────────────────────────────────────────────────────────────
\section{Augmented-Lagrangian Constraint Enforcement (DRO-FAIR-AL)}
\label{sec:report-al}
% ─────────────────────────────────────────────────────────────────────────────

\textbf{Diagnosis.} The dual step's geometric decay
($\mathrm{lr}_\lambda\cdot0.95^{\mathrm{epoch}}$) caps $\lambda$'s total
accumulation over 60 epochs at $\approx0.1$ regardless of $\lambda_{\max}$.
Measured on Adult $\alpha{=}0.2$ seed 0: $\lambda_{\mathrm{dp}}$ peaks at
\textbf{0.0119} against a ceiling of $1.5$, and the fairness penalty
$\lambda\cdot g$ peaks at \textbf{0.0029} against a training loss of
$0.538$ --- about $0.5\%$ of the loss. This is why raising $\lambda_{\max}$
$1.5\to2.0$ was a byte-identical no-op (\S\ref{sec:results-al} of the paper;
Future Work item (iii) below): the ceiling was never binding.

\textbf{Fix.} Add the classical augmented-Lagrangian quadratic penalty:
$\mathcal{L}_{\mathrm{AL}} = \mathcal{L}_{\mathrm{tilt}} + \lambda g +
\tfrac{\mu}{2}g^2$ per constraint. $\mu{=}0$ recovers the canonical
objective exactly (unit-tested bit-identical), so all results elsewhere in
this report are unaffected.

\textbf{Result (Adult, DP attack, $n{=}6$, seed-paired Wilcoxon).} A
90-run $\mu$-sensitivity sweep found $\mu{=}20$ maximises the 6-seed-mean DP
reduction while keeping the mean accuracy above the safety threshold:

\begin{center}
\begin{tabular}{lrrrr}
\toprule
$\alpha$ & DP: Naive$\to$DRO$\to$AL & $p$ & acc: DRO$\to$AL & margin vs.\ Naive \\
\midrule
...
\textbf{Scope, stated directly.} A pre-registered $\alpha{=}0$ falsification
control found AL reduces DP \emph{more} without corruption than under
attack, so this is a general fairness-optimisation fix, not a
corruption-robustness mechanism. At $\alpha{=}0.4$, no $\mu\in\{0.5,1,2,5,10,20\}$
is safe --- the most aggressive value gave the best-looking DP number in the
whole study ($-80.7\%$) while accuracy collapsed to $0.3495$, below chance.
Credit is not rescued by any tested $\mu$. A further test found AL
\textbf{conflicts} rather than compounds with a separately-observed radius
under-calibration (most cells prefer $\mathrm{radii\_scale}{=}2.0$ over the
canonical $1.0$): stacking $\mu{=}20$ with the larger radius collapses
accuracy to $0.7561$ at $\alpha{=}0.2$, at the degeneracy threshold, turning
its apparent $-94.0\%$ DP number into constant-predictor collapse rather than
a stronger result. AL is recommended for \textbf{Adult, $\alpha\le0.2$, the
canonical radius, only}; all three boundaries are reported as findings.
Supplementary evidence at $\mu{=}5$ (the originally-discovered value): DP win
transfers to the Combined attack ($-46.8\%$, $p{=}0.0156$, accuracy
\emph{improving}); LSAC collapses under AL as it already does under DRO
(\S\ref{sec:lsac-degen} -- pre-existing degeneracy, not a new failure).

\textbf{Independent review (\texttt{docs/AL\_REVIEW.md}).} A separate
adversarial pass -- fresh, no history on this project, re-deriving every
number from raw JSON and live reruns rather than trusting the summaries
above -- confirmed the core statistics (DP reduction, $p{=}0.015625$, the
$\mu{=}0$ no-op, the $\lambda$-starvation diagnosis, no code-path leakage)
but found the accuracy framing overstated: at $\alpha{=}0$ accuracy
\emph{drops} in all 6 seeds ($0.8147\to0.7966$), not ``held or improved'';
and at $\alpha{=}0.2$ one of the six seeds ($\mu{=}20$, seed 3) is a full
constant-negative predictor (accuracy exactly the test majority rate),
hidden by the 6-seed mean. Direct per-seed reruns (seeds 0--3) confirm this
predicted-positive-rate collapse is general at $\mu{=}20$, not specific to
one seed -- \emph{AL pushing predictions toward the majority class} is the
accurate mechanism, not attack-specific denoising alone. The DP result and
its significance stand; the accuracy side is a genuine, seed-dependent cost
in places, reported here rather than smoothed into an aggregate mean.
Full derivation, pre-registration, and all raw results:
\texttt{docs/superpowers/specs/2026-08-05-augmented-lagrangian-design.md},
\texttt{results/aug\_lagrangian\_extended\_summary.md},
\texttt{results/mu\_sensitivity\_summary.md}.
...
\item \textbf{Reproducibility re-run under the corrected (cosine) IF metric.}
  The locked grid predates a $k$-NN metric alignment; its DP/COMBINED rows
  record an IF violation of $\approx10^{-11}$ (noise, not a measurement), so
  no IF effect size is quoted there. A full 540-row re-run under the
  corrected metric
  (\texttt{results/canonical\_tau1\_cosine.json}) makes the IF column a real
  measurement and reproduces the DP- and IF-attack rows closely (max
  $|\Delta \mathrm{acc}|\le0.005$, max $|\Delta \mathrm{DP}|\le0.008$),
  but two $n{=}6$ cells lose significance in the re-run: Adult/DP
  $\alpha{=}0.1$ ($5/6$, $p{=}0.031\to4/6$, $p{=}0.109$) and LSAC/Combined
  $\alpha{=}0.1$ ($6/6$, $p{=}0.016\to4/6$, $p{=}0.078$). Combined-attack
  rows drift at $\alpha\ge0.2$ (max $|\Delta \mathrm{acc}|=0.075$, max
  $|\Delta \mathrm{DP}|=0.085$) because the corrected metric changes which
  samples the combined attack corrupts. This report keeps the locked grid
  as its source; the two $\alpha{=}0.1$ cells are flagged as marginal rather
  than clean wins. Full details: \texttt{results/task\_f\_verification\_summary.md}.
\end{enumerate}
...
\texttt{results/fairness\_aggressiveness\_summary.md}. We diagnose the
underlying defect as $\lambda$-starvation --- the geometrically-decaying
dual learning rate caps $\lambda$'s total accumulation at $\approx0.1$,
so the fairness penalty never exceeds $\approx0.5\%$ of the loss
(\S\ref{sec:report-al}) --- and resolve it with a penalty-only
augmented Lagrangian ($\mu{=}20$, canonical radius) in
\S\ref{sec:report-al}. This resolves the headline mechanism; what it
leaves open is scoped and listed next.

\textbf{(iv) Open extensions of the augmented Lagrangian.} The AL
formulation of \S\ref{sec:report-al} is demonstrated on a single
dataset (Adult), a single corruption regime ($\alpha\le0.2$), and a
single radius choice. Three concrete next steps follow.
\emph{Adaptive $\mu$ scheduling}: the safe $\mu$ is a function of
$\alpha$ ($\mu{=}20$ is safe at $\alpha\le0.2$ but collapses the model
at $\alpha{=}0.4$), so a schedule that decays $\mu$ as $\alpha$ grows
would extend the method's usable range without a separate sweep per
corruption level. \emph{Multi-group / multi-attribute settings}: all
current AL results are on a binary protected attribute; whether the same
$\mu$ is safe when several group-rate gaps are penalised simultaneously
is untested. \emph{Understanding the AL $\times$ radius conflict}:
stacking $\mu{=}20$ with the larger radius that the radius ablation
independently prefers collapses accuracy rather than compounding the gain
(\S\ref{sec:report-al}) --- the mechanism of that conflict is not yet
diagnosed.
