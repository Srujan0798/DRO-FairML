...
\item \textbf{UTKFace partial (no paper claim yet).}
  Real features exist (\texttt{data/raw/utkface\_features.npz}); a partial
  REAL-tagged grid is in \texttt{results/utkface\_canonical.json} (DP
  attack only so far; not 90/90).  Prior synthetic smoke numbers and the
  ``DRO inverts on image features'' claim remain withdrawn until a full
  multi-seed grid is complete and reviewed.
\end{enumerate}
