...
Prints the key numbers for the 4 PM meeting in a copy-paste-friendly
format. Pulls all numbers directly from the result files — no hand-typed
stats.
...
def _load_tau1():
    canonical = _load("tau_ablation_tau1.json")
    if len(canonical) >= 30:
        return canonical, "canonical"
    bak = _load("tau_ablation_tau1_KINNER5_BAK.json")
    if bak:
        return bak, "FALLBACK K_inner=5"
    return canonical, "canonical"
...
def main():
    ...
    if not args.no_banner:
        ...
        print("Source: results/tau_ablation_*.json, results/knn_ablation_*.json, "
              "results/random_vs_adversarial_new.json")

    tau1, tau1_src = _load_tau1()
    tau10  = _load("tau_ablation_tau10.json")
    tau100 = _load("tau_ablation_tau100.json")
    knn    = (_load("knn_ablation_k5.json")
              + _load("knn_ablation_k10.json")
              + _load("knn_ablation_k15.json"))
    rvsa   = _load("random_vs_adversarial_new.json")

    print(f"\nTau=1 data source: {tau1_src} ({len(tau1)} rows)")

    # ---- 1. HEADLINE: DP attack, fixed tau=1, by alpha ---------------------
    header("1. HEADLINE — DP attack, fixed tau=1 (vs old tau=10/100)")
    print(f"{'alpha':>6} | {'Naive DP':>22} | {'DRO DP':>22} | {'winner':>8} | {'DRO wins / n'}")
    print("-" * 78)

    # For each tau, for the chosen dataset, DP-attack rows
    for tau_name, tau_rows in [("tau=1", tau1), ("tau=10", tau10), ("tau=100", tau100)]:
        sub = [r for r in tau_rows
               if r["dataset"] == args.dataset and r["attack"] == "dp"]
        if not sub:
            continue
        print(f"\n  {tau_name}:")
    ...
    header("2. IF and Combined attacks, fixed tau=1")
    ...
    # ---- 3. IF k-NN ablation ------------------------------------------------
    header("3. IF k-NN ablation (insensitivity check)")
    if knn:
    ...
    header("4. Random vs adversarial — absolute DP")
    if rvsa:
        for ds in ["adult", "credit", "lsac"]:
            ...
            for a in sorted({r["alpha"] for r in sub}):
                ...
                # Adv/random ratio: (adv - clean) / (rand - clean)
                dc = r_m - c_m
                da = a_m - c_m
                if dc > 1e-4 and da > 0:
                    ratio = f"{da / dc:.1f}×"
        ...
        print("  no random-vs-adversarial data")

    # ---- 5. Lambda grid (in progress) ---------------------------------------
    header("5. Lambda init × lr_lambda grid (Kuldeep Q1, in progress)")
    grid = _load("lambda_lr_grid.json")
    if grid:
        print(f"  {len(grid)}/72 cells complete")
        if grid:
            r = grid[0]
            print(f"  Latest cell: alpha={r['alpha']} seed={r['seed']} "
                  f"lambda_init={r['lambda_init']} lr_lambda={r['lr_lambda']} "
                  f"-> acc={r['acc']:.3f} dp={r['dp']:.4f}")
    else:
        print("  no grid data yet")
