#!/usr/bin/env python3
"""Generate IF violation vs alpha plots broken down by attack type (Kuldeep request)."""
import json, os, sys
import numpy as np
import pandas as pd
import matplotlib
...
for attack in ['dp', 'if', 'combined']:
    ...
    for meth, marker in [("naive", "o"), ("dro", "s")]:
        ...
        for alpha in sorted(set(r['alpha'] for r in sub)):
            vals = [r['if_clean'] for r in sub if r['method'] == meth and r['alpha'] == alpha]
            vals = np.array(vals)
            if len(vals):
    ...
    ax.set_title(f"Adult IF violation vs $\\alpha$ — {attack.upper()} attack ($\\tau = 1$)")
    ax.legend(loc="upper right", fontsize=9)
    ax.set_xticks(sorted(set(r['alpha'] for r in sub)))
    fig.tight_layout()
    stem = f"adult_if_{attack}_attack_tau1_meeting"
    savefig(fig, stem)

print("Done. Saved adult_if_{dp,if,combined}_attack_tau1_meeting.{pdf,png}")
