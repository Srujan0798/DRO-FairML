...
def run_ablation(dataset_name, alpha, seed, device='cpu'):
    ...
    trainer.fit(X_tr, y_tr, X_va, y_va, verbose=False)
    ...
    # 2. Naive-FAIR
    model = MLPClassifier(input_dim, hidden_dims=[64, 32], dropout=0.1)
    trainer = NaiveFairTrainer(model, device=device, epochs=30, tau=100.0, k=5)
    trainer.fit(X_tr, y_tr, a_tr, X_va, y_va, a_va, verbose=False)
    preds = trainer.predict(X_test)
    results['naive'] = {
        'accuracy': float(compute_accuracy(y_test, preds)),
        'dp_violation': float(compute_dp_violation(preds, a_test)),
    ...
    # 3. DRO-FAIR joint
    model = MLPClassifier(input_dim, hidden_dims=[64, 32], dropout=0.1)
    trainer = DroFairTrainer(model, alpha=alpha, device=device, epochs=30, tau=100.0, k=5,
                             K_inner=5, lr_p=5e-3, use_dp=True, use_if=True)
    trainer.fit(X_tr, y_tr, a_tr, X_va, y_va, a_va, verbose=False)
    preds = trainer.predict(X_test)
    results['dro_joint'] = {
        'accuracy': float(compute_accuracy(y_test, preds)),
    ...
    model = MLPClassifier(input_dim, hidden_dims=[64, 32], dropout=0.1)
    trainer = DroFairTrainer(model, alpha=alpha, device=device, epochs=30, tau=100.0, k=5,
                             K_inner=5, lr_p=5e-3, use_dp=True, use_if=False)
    trainer.fit(X_tr, y_tr, a_tr, X_va, y_va, a_va, verbose=False)
    preds = trainer.predict(X_test)
    results['dro_dp_only'] = {
        'accuracy': float(compute_accuracy(y_test, preds)),
    ...
    model = MLPClassifier(input_dim, hidden_dims=[64, 32], dropout=0.1)
    trainer = DroFairTrainer(model, alpha=alpha, device=device, epochs=30, tau=100.0, k=5,
                             K_inner=5, lr_p=5e-3, use_dp=False, use_if=True)
    trainer.fit(X_tr, y_tr, a_tr, X_va, y_va, a_va, verbose=False)
    preds = trainer.predict(X_test)
    results['dro_if_only'] = {
        'accuracy': float(compute_accuracy(y_test, preds)),
