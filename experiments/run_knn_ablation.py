#!/usr/bin/env python3
"""IF k-NN ablation: test different k values for IF attack."""
...
def main():
    ...
    parser.add_argument('--k', type=int, required=True, help='k-NN neighborhood size')
    ...
    parser.add_argument('--attacks', nargs='+', default=['if'])  # IF attack only
    ...
    # Monkey-patch FairnessTargetedPGD to use our k
    from src.corruption.adversarial import FairnessTargetedPGD
    original_init = FairnessTargetedPGD.__init__
    def patched_init(self, *init_args, **init_kwargs):
        init_kwargs['k'] = args.k
        original_init(self, *init_args, **init_kwargs)
    FairnessTargetedPGD.__init__ = patched_init

    device = 'cuda' if torch.cuda.is_available() else 'cpu'
    out_path = f'results/knn_ablation_k{args.k}.json'
            ...
            for seed in range(args.n_seeds):
                for attack in args.attacks:
                    for method in args.methods:
                        ...
                        print(f"[{count}/{total}] {dataset} α={alpha} s={seed} {attack} {method} (k={args.k})")
                        try:
                            t0 = time.time()
                            ...
                            print(f"  → acc={result['acc_clean']:.3f} dp={result['dp_clean']:.4f} if={result['if_clean']:.4f} ({elapsed:.0f}s)")
