...
def main():
    ...
    lines = ["# DRO-FAIR-AL (augmented Lagrangian) — pre-registered result",
             ...
             "DP naive | margin dro | margin AL | acc dro | acc AL |",
             "|---|---|---|---|---|---|---|---|---|---|---|---|---|"]
    ...
    for mu in MUS:
        ...
        verdicts.append(f"- μ={mu:g}: {sig_cells[mu]}/4 cells significant; "
                        f"mean acc cost {cost:+.4f} → "
                        f"{'**CRITERION MET**' if ok else 'criterion NOT met'}")
    lines += verdicts
    lines.append("")
