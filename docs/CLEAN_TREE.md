...
| Layout (one screen) | `docs/REPO_LAYOUT.md` (if present) |

## Active code
| Dir | Keep (role) |
|-----|-------------|
| `src/` | Trainers, `FairnessTargetedPGD`, metrics, models, radii, `temperature.py` (τ=1) |
| `experiments/` | Canonical pipeline + Makefile artifact gens + UTKFace + headline plots |
...
- Meeting / figD / fig_final / heatmaps kept as PDF; duplicate PNG removed when PDF exists
- PNG-only kept: `fig_win_curves_tau1.png`
