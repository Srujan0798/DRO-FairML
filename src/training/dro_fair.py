                ...
                for _ in range(self.K_inner):
                    if self.use_dp:
                        for j in [0, 1]:
                            ...
                            p_temp[j] = p_j
                            dp_loss = self._compute_dp_loss_weighted(h_tilde.detach(), a_t, p_temp, group_mask_dict)
                            dp_loss.backward()
                    ...
                    if self.use_if:
                        ...
                        if_loss = self._compute_if_loss_weighted(h_tilde.detach(), p_if_grad, edge_i, edge_j, edge_dists)
                        if_loss.backward()
