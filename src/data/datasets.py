...
def _download_file(url, path):
    """Download file using curl (works around Python SSL issues)."""
...
def _generate_synthetic_adult(n_samples=45222, n_features=12, random_state=42):
    ...
    # Protected attribute: sex (Male=1, Female=0), ~33% Male
    a = (rng.rand(n_samples) > 0.67).astype(np.int64)
    
    X = np.zeros((n_samples, n_features), dtype=np.float32)
    
    # Group-dependent features (realistic: different means for different groups)
    for i in range(n_features):
        X[a == 0, i] = rng.normal(loc=0.0, scale=1.0, size=(a == 0).sum())
        X[a == 1, i] = rng.normal(loc=0.3, scale=1.0, size=(a == 1).sum())
    
    # Label with group-dependent threshold (creates natural DP disparity)
...
def load_credit(data_dir='data/raw'):
    """Load and preprocess Credit Card Default dataset."""
    os.makedirs(data_dir, exist_ok=True)
...
def load_lsac(data_dir='data/raw'):
    """Load and preprocess LSAC dataset."""
    os.makedirs(data_dir, exist_ok=True)
    ...
    try:
        if not os.path.exists(path):
            ...
            urls = [
                'https://raw.githubusercontent.com/damtharvey/lsac-dataset/main/lsac.csv',
                'https://raw.githubusercontent.com/propublica/compas-analysis/master/lsac.csv',
            ]
            for url in urls:
                try:
        ...
        y = (df['pass_bar'].values == 1).astype(np.float32)
        
        # Protected attribute: gender (1=male, 0=female)
        a = (df['gender'].values == 1).astype(np.int64)
        
        # Drop target and protected
        df = df.drop(columns=['pass_bar', 'gender'])
